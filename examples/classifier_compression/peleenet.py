...
class PeleeNet(nn.Module):
    def __init__(self,num_classes=3341, num_init_features=32,growthRate=32, nDenseBlocks = [3,4,8,6], bottleneck_width=[1,2,4,4]):
    ...
    def _make_dense_transition(self, dense_inp,total_filter, inter_channel, ndenseblocks,with_pooling= True):
        ...
        for i in range(ndenseblocks):
            ...
            layers.append(DenseBlock(dense_inp, inter_channel, self.half_growth_rate*2))
            dense_inp += self.half_growth_rate * 2
...
if __name__ == '__main__':
    ...
    pelee = PeleeNet(num_classes=3341).to(device)
    # input = torch.autograd.Variable(torch.ones(1, 3, 224, 224))
    # output = pelee(input)
    # with SummaryWriter(comment='peleenet') as w:
    #     w.add_graph(pelee, (input, ))
    # summary(pelee, (3, 224, 224))
    for name, parameters in pelee.named_parameters():
        if 'weight' in name:
            print(name)